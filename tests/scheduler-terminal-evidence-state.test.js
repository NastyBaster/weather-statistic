...
function binding() { const b = { state: { phase: "read_only_negative_evidence_required", negative: [{ label: "A", status: 405, category: "method_not_allowed" }], attempt_boundary: "2026-01-01T00:00:00Z", scheduled_run_baseline: 0 }, writes: [], clears: 0,
  async readPhaseState() { return this.state; }, async clearWriteArtifacts() { this.clears++; }, async writePhaseState(s) { this.writes.push(s); this.state = s; }, async preflight() { throw new Error("must not preflight"); }, async writeSqlArtifacts() { throw new Error("must not enqueue"); } }; return b; }
const b = binding();
...
console.log("scheduler terminal evidence state: 7 fixtures, 0 failed, 0 skipped, 0 not-run");
