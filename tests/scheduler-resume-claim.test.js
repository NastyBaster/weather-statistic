...
const fs = {
  async mkdir(_path) { if (files.has("scheduler-resume-claim")) { const e = new Error("exists"); e.code = "EEXIST"; throw e; } files.add("scheduler-resume-claim"); },
  async lstat(path) { return { isSymbolicLink: () => false, isDirectory: () => path.endsWith("validation") }; },
