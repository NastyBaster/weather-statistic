...
The reviewed function is deployed to production and its manual invocation, idempotency, immutability, and RLS boundaries are validated. It remains manual-only: scheduling, a dashboard trigger, per-location diagnostics, observations, accuracy calculations, and real-data UI are deliberately deferred. The current UI still uses demonstration data. Sanitized rollout evidence and the narrow Dashboard-only log review are recorded in [`../validation/manual-forecast-collector-production.md`](../validation/manual-forecast-collector-production.md).
