...
if (command === "doctor") { const result = await runDoctor(createRealDoctorAdapter(), runtimeRoot()); console.log(JSON.stringify({ command: "doctor", pass: result.pass, failures: result.failures, checks: result.checks })); process.exitCode = result.pass ? 0 : 1; }
